...
//todo: rename to make it obvious that this is not just a getter but also a mutator that may create a new pool if necessary
...
void DescriptorAllocatorGrowable::init(VkDevice device,
	uint32_t maxSets,
	std::span<PoolSizeRatio> poolRatios) {
	LOG_INFO("DescriptorAllocatorGrowable::init called with maxSets: {}", maxSets);
...
void DescriptorWriter::write_buffer(int binding, 
	VkBuffer buffer,
	size_t size,
	size_t offset,
